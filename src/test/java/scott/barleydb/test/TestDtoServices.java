...
public class TestDtoServices extends TestBase {
  ...
  public void testSaveTemplateDatatype() throws SortServiceProviderException, SortPersistException, BarleyDBQueryException {
    ...
    stats = etlServices.getLastCtxStatistics();
    Assert.assertEquals(2, stats.getNumberOfQueries()); //queries loading original data for comparison
    Assert.assertEquals(2, stats.getNumberOfQueryDatabseCalls()); //queries loading original data for comparison
    Assert.assertEquals(0, stats.getNumberOfRecordInserts());
